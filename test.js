...
var getTimezoneOffset = require('./getTimezoneOffset.js');

test('getTimezoneOffset', function(t) {
	...
	tzinfo.forEach( function( scenario ) {
		var tz_name = scenario[0];
		var instant = new Date( scenario[1] );
		var expect_minutes = scenario[2];
		t.equal( getTimezoneOffset( tz_name, instant ), expect_minutes,
			tz_name + " at " + instant.toISOString() );
	});
